...
def train_xgboost_model(stock_symbol: str, data: pd.DataFrame, target_column: str = None,
    ...
    native_importance = get_xgboost_feature_importance(best_model, X.columns)
    shap_importance = calculate_xgboost_shap_importance(best_model, X, X.columns)
...
def calculate_xgboost_shap_importance(model, X, feature_names) -> Dict[str, float]:
    ...
    """
    try:
        # Calculate SHAP values - XGBoost has a specific explainer
