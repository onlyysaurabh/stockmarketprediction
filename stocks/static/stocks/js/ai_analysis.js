...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Get current stock symbol from URL path
    const pathParts = window.location.pathname.split('/');
    const stockSymbol = pathParts[pathParts.length - 1];
    ...
    async function analyzeStock(analyzer, forceRefresh = false) {
        ...
        try {
            ...
            let apiUrl = forceRefresh ? 
                ...
                `/stocks/ajax/analyze/${stockSymbol}/`;
    ...
    function displayAnalysisResult(data) {
        ...
        if (data.error) {
            ...
            // Update source text
            const analyzerName = getAnalyzerDisplayName(data.analyzer_used);
